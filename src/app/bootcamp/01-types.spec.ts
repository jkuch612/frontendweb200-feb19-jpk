...
describe('declaring variables etc with typescript', () => {
  describe('eventual typing', () => {
    ...
    it('an example of implicit typing', () => {
      let age = 49;
...
describe('tuples', () => {
  ...
  it('a couple fo pieces bfore we look at a tuple', () => {
  ...
  it('formatting a name with a tuple', () => {
    ...
    const n1 = answer[0];
    const n2 = answer[1];
    expect(n1).toBe('Skywalker, Luke');
    expect(n2).toBe(15);

    const [fn, count] = formatName('Han', 'Solo');
    expect(fn).toBe('Solo, Han');
    expect(count).toBe(9);

    const todos = ['Clean Garage', 'Fix Laundry Room Light', 'Fix Downspout'];

    const [next, , end] = todos;
    expect(next).toBe('Clean Garage');
    expect(end).toBe('Fix Downspout');
...
describe('object literals', () => {
  it('has them', () => {
    ...
    const henry = {
      ...
      lastName: 'Gonzalez',
      age: 7,
      hobby: 'stmap collecting'
    };

    // accept sthis because of duck typing even though Henry is untyped and has extra properties. It just has to have the minimum properties
